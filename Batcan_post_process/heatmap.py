...
#%%
path = 'D:/projects/Data/Batcan/summary_smax.csv'
...
bulkdata['density'] = 1.0869 +934.1*bulkdata["Thickness"]*(1-0.5)+2216**bulkdata["Thickness"]*(0.5)
...
# d_max = bulkdata['Delta'].max() #W/m2
# d_min = bulkdata['Delta'].min()
...
#%%
#For Four current plots

pointer = np.arange(0.35,0.65,0.05)[::-1].round(2)
...
for n, current in enumerate(array):
    ...
    # Delta = data.pivot('Porosity [-]', 'Thickness (μm)', "Delta")
    ...
    # sns.heatmap(Delta, ax=axes4[a,b], cmap="seismic").set_title(current ) #, vmin=-1*d_max, vmax=d_max
...
fig.savefig('graded_pc'+ '.png', format='png', dpi=300, bbox_inches = "tight")
...
for n, current in enumerate(array):
    ...
    sns.heatmap(energy_density, ax=axes[a,b]).set_title(current + ': Max Energy Density =' + str(round(ed_max)) + ' Wh kg⁻¹') #, vmin=ed_min, vmax=ed_max
    sns.heatmap(power_density, ax=axes2[a,b]).set_title(current + ': Max Power Density =' + str(round(pd_max,1)) + ' W kg⁻¹') #, vmin=pd_min, vmax=pd_max
    sns.heatmap(Delta, ax=axes[1,b], cmap="seismic", vmin=1*d2_max, vmax=-d2_max).set_title(current ) #, vmin=-1*d_max, vmax=d_max  vmin=1*d2_max, vmax=-d2_max
    sns.heatmap(GHG, ax=axes3[a,b], cmap="crest").set_title(current + ': Min GHG =' + str(round(g_min)) + ' kg CO2 kWh⁻¹') #, vmin=pd_min, vmax=pd_max
